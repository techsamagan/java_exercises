...
public class FilterExercise {
    ...
    public static void main(String[] args) {
        ...
        people.stream().filter(p -> p.age > 18).collect(Collectors.toList());


        // TODO: 2 - Filter 'orders' with amount greater than 100
        //           Print each order's product and amount

        orders.stream().filter(order -> order.amount > 100).collect(Collectors.toList());
        ...
        people.stream().filter(person -> person.age > 18 && person.city.equals("London")).collect(Collectors.toList());
        ...
        people.stream()
                .filter(person -> "Paris".equals(person.city) || person.age < 18)
                .collect(Collectors.toList());
        // TODO: 7 - Count the number of COMPLETED orders
        //           Use filter + count() and print the result

        orders.stream().filter(order -> order.status.equals("COMPLETED")).count();
